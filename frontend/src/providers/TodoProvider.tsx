...
export default function TodosProvider({ children }: any) {
  ...
  const [showDescription, setShowDescription] = useState<[boolean, TodoType]>([false, {} as TodoType]);
  const [alertChange, setAlertChange] = useState<boolean>(false);

  return (
    <TodosContext.Provider
      value={{ todosList, setTodosList, showDescription, setShowDescription, alertChange, setAlertChange }}
    >
      {children}
    </TodosContext.Provider>
  );
